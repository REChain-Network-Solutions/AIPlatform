...
<li
	class="lqd-navbar-item lqd-navbar-label-collapsible relative group/nav-item"
	x-data="{
...
>
	<span
		class="lqd-navbar-label-wrap flex w-full min-w-0 cursor-pointer items-center gap-2 pb-navbar-link-pb pe-navbar-link-pe ps-navbar-link-ps pt-navbar-link-pt"
		@click="toggle()"
	>
		<span class="lqd-navbar-label inline-block max-w-full min-w-0 flex-1 overflow-hidden text-ellipsis text-4xs uppercase tracking-widest lg:group-[&.navbar-shrinked]/body:w-full lg:group-[&.navbar-shrinked]/body:px-2 lg:group-[&.navbar-shrinked]/body:text-center">
	...
	<ul
		class="lqd-navbar-label-children"
		x-show="isOpen"
		x-collapse
	>
		@foreach ($item['children'] as $child)
