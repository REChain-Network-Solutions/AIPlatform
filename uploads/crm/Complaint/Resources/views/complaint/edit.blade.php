...
@section('filter-section')
    ...
    <div class="d-flex px-4 filter-box bg-white">
        ...
        @if ($viewComplaintPermission == 'all' || ($viewComplaintPermission == 'owned' && $complaint->agent_id == user()->id))
            <div id="ticket-closed" @if ($complaint->status == 'closed') style="display:none" @endif>
    ...
    <div class="ticket-wrapper bg-white border-top-0 d-lg-flex">
        ...
        <div class="ticket-left w-100">
            <x-form id="updateTicket2" method="PUT">
                ...
                <input type="hidden" id="complaint_reply_id" value="">

                <!-- START -->
                <div class="d-flex justify-content-between align-items-center p-3 border-right-grey border-bottom-grey">
                    <span>
                        <p class="f-15 f-w-500 mb-0">{{ $complaint->subject }}</p>
                        <p class="f-11 text-lightest mb-0">@lang('modules.tickets.requestedOn')
                            {{ $complaint->created_at->timezone(company()->timezone)->translatedFormat(company()->date_format . ' ' . company()->time_format) }}
                        </p>
                    </span>
                    <span>
                        ...
                        <p class="mb-0 text-capitalize ticket-status">
                            <x-status :color="$statusColor" :value="__('app.' . $complaint->status)" />
                        </p>
                    </span>
                </div>
                <!-- END -->
                ...
                <div class="ticket-msg border-right-grey" data-menu-vertical="1" data-menu-scroll="1"
                    ...
                    @foreach ($complaint->reply as $reply)
                ...
                <div class="col-md-12 border-top border-right d-none mb-5" id="reply-section">
                    <div class="form-group my-3">
                        @if ($complaint->requester->id != user()->id || (!is_null($complaint->agent_id) && $complaint->agent_id != user()->id))
                            <p class="f-w-500">
                                @lang('app.to'):
                                {{ $complaint->requester->id != user()->id ? $complaint->requester->name : $complaint->agent->name }}
                            </p>
                        @endif
                ...
                <div class="ticket-reply-back justify-content-start px-lg-4 px-md-4 px-3 py-3  d-flex bg-white border-top-grey border-right-grey"
                    ...
                    <x-forms.link-secondary :link="route('complaint.index')" icon="arrow-left">@lang('app.back')
                ...
                <div class="ticket-reply-back flex-row justify-content-start px-lg-4 px-md-4 px-3 py-3 c-inv-btns bg-white border-top-grey border-right-grey d-none"
                    ...
                    @if ($viewComplaintPermission == 'all' || $complaint->agent_id == user()->id)
        ...
        <!-- TICKET LEFT END -->
        @if ($viewComplaintPermission == 'all')
            <!-- TICKET RIGHT START -->
                        ...
                        <x-form id="updateTicket1">
                            ...
                            <div class="ticket-filters p-4 w-100 position-relative border-bottom">
                                <div class="more-filter-items">
                                    ...
                                    @if ($wr->isEmpty())
                                        <a href="{{ route('complaint.createWR', [$complaint->id, $complaint->complaint_number]) }}"
                                ...
                                @if (!$wr->isEmpty())
                                    <div class="more-filter-items">
                                        ...
                                        @if (!$wr->isEmpty() && $wo->isEmpty())
                                            <a href="{{ route('complaint.createWO', [$complaint->id, $complaint->complaint_number, $wr[0]->id]) }}"
                                                class="btn btn-outline-secondary border-grey">@lang('app.add')
                                                    ...
                                                    <optgroup label="{{ mb_ucwords($group->group_name) }}">
                                                        @foreach ($group->enabledAgents as $agent)
                                                            <x-user-option :user="$agent->user"
                                                                :selected="$agent->user->id == $complaint->agent_id"></x-user-option>
                                                        @endforeach
                                                    </optgroup>
                                                @endif
                                            @endforeach
                                ...
                                <div class="more-filter-items">
                                    <x-forms.select fieldId="priority" :fieldLabel="__('modules.tasks.priority')" fieldName="priority"
                                        data-container="body">
                                        <option @if ($complaint->priority == 'low') selected @endif value="low">
                                            @lang('app.low')</option>
                                        <option @if ($complaint->priority == 'medium') selected @endif value="medium">
                                            @lang('app.medium')</option>
                                        <option @if ($complaint->priority == 'high') selected @endif value="high">
                                            @lang('app.high')</option>
                                        <option @if ($complaint->priority == 'urgent') selected @endif value="urgent">
                                    ...
                                    <x-forms.input-group>
                                        <select class="form-control select-picker" name="type_id" id="ticket_type_id"
                                            ...
                                            @foreach ($types as $type)
                                                <option @if ($type->id == $complaint->type_id) selected @endif
                                                    value="{{ $type->id }}">
                                    ...
                                    <x-forms.input-group>
                                        <select class="form-control select-picker" name="channel_id"
                                            ...
                                            @foreach ($channels as $channel)
                                                <option @if ($channel->id == $complaint->channel_id) selected @endif
                                                    value="{{ $channel->id }}">
                                ...
                                <div class="more-filter-items">
                                    ...
                                    <input type="text" name="tags" id="tags" class="rounded f-14"
                                        value="{{ implode(',', $complaint->complaintTags->pluck('tag_name')->toArray()) }}">
                                </div>
                            </div>
                            <!-- TICKET FILTERS END -->
                            <!-- TICKET UPDATE START -->
                    ...
                    <div class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab">
                        ...
                        <div class="card-horizontal bg-white-shade ticket-contact-owner p-4 rounded-0">
                            <div class="card-img mr-3">
                                <img class="___class_+?88___" src="{{ $complaint->requester->image_url }}"
                                    alt="{{ mb_ucwords($complaint->requester->name) }}">
                            </div>
                            <div class="card-body border-0 p-0 w-100">
                                <h4 class="card-title f-14 font-weight-normal mb-0">
                                    <a class="text-dark-grey"
                                        @if ($complaint->requester->hasRole('employee')) href="{{ route('employees.show', $complaint->requester->id) }}"
                                    @else
                                        href="{{ route('clients.show', $complaint->requester->id) }}" @endif>
                                        {{ mb_ucwords($complaint->requester->name) }}
                                    </a>
                                </h4>
                                @if ($complaint->requester->country_id)
                                    <span class="card-text f-12 text-dark-grey text-capitalize d-flex align-items-center">
                                        <span
                                            class='flag-icon flag-icon-{{ strtolower($complaint->requester->country->iso) }} mr-2'></span>
                                        {{ $complaint->requester->country->nicename }}
                                    </span>
                                @else
                                    --
                                @endif
                        ...
                        <x-cards.data :title="__('app.menu.tickets')" padding="false">
                            <x-pie-chart id="ticket-chart" :labels="$complaintChart['labels']" :values="$complaintChart['values']" :colors="$complaintChart['colors']"
                            ...
                            <div class="card-body p-0">
                                <div class="recent-ticket position-relative" data-menu-vertical="1" data-menu-scroll="1"
                                    ...
                                    <div class="recent-ticket-inner position-relative">
                                        @foreach ($complaint->requester->tickets as $item)
                                            <div class="r-t-items d-flex">
                                                ...
                                                <div class="r-t-items-right ">
                                                    <h3 class="f-14 font-weight-bold">
                                                        <a class="text-dark"
                                                            href="{{ route('complaint.show', $item->id) }}">{{ $item->subject }}</a>
                                                    </h3>
                                                    <span class="d-flex mb-1">
                                                        <span
                        ...
                        <x-form id="updateOther">
                            ...
                            <div class="ticket-filters p-4 w-100 position-relative border-bottom">
                                <x-forms.custom-field-show :fields="$fields"
                                    :model="$complaint"></x-forms.custom-field-show>
                            </div>
                            <!-- TICKET FILTERS END -->
                        </x-form>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        taskDropzone.on('sending', function(file, xhr, formData) {
            var ids = $('#complaint_reply_id').val();
        ...
        taskDropzone.on('completemultiple', function() {
            ...
            window.location.href = "{{ route('complaint.show', $complaint->complaint_number) }}";
        });
        ...
        $('.submit-ticket').click(function() {
            ...
            const url = "{{ route('complaint.update', $complaint->id) }}";
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        if (taskDropzone.getQueuedFiles().length > 0) {
                            $('#complaint_reply_id').val(response.reply_id);
                            taskDropzone.processQueue();
                        } else {
                            window.location.href =
                                "{{ route('complaint.show', $complaint->complaint_number) }}";
                        }
                    }
                }
            });
        ...
        $('.submit-ticket-2').click(function() {
            ...
            $.easyAjax({
                url: "{{ route('complaint.update_other_data', $complaint->id) }}",
        ...
        $('.apply-template').click(function() {
            ...
            $.easyAjax({
                url: "{{ route('complaintTemplates.fetchTemplate') }}",
                data: {
        ...
        $('body').on('click', '.delete-file', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('complaint-files.destroy', ':id') }}";
        ...
        $('body').on('click', '.delete-message', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('complaint-replies.destroy', ':id') }}";
                    url = url.replace(':id', id);
