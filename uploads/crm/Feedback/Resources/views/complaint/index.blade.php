            ...
            <div class="select-status">
                <select class="form-control select-picker" name="status" id="ticket-status">
                    ...
                    <option {{ request('status') == 'open' || request('status') == '' ? 'selected' : '' }}
                        value="open">
                        @lang('feedback::modules.totalOpen')</option>
                    <option {{ request('status') == 'pending' ? 'selected' : '' }} value="pending">
                        @lang('feedback::modules.totalPending')</option>
                    <option {{ request('status') == 'resolved' ? 'selected' : '' }} value="resolved">
                        @lang('feedback::modules.totalResolved')</option>
                    <option {{ request('status') == 'closed' ? 'selected' : '' }} value="closed">
                        @lang('feedback::modules.totalClosed')</option>
                </select>
            </div>
        </div>
        <!-- STATUS END -->
        ...
        <x-filters.more-filter-box>
            ...
            @if (!in_array('client', user_roles()))
                <div class="more-filter-items">
                    <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('feedback::modules.feedbackAgents')</label>
                ...
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" name="priority" id="priority" data-live-search="true"
                            ...
                            <option value="urgent">@lang('feedback::modules.urgent')</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('feedback::modules.feedbackChannels')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('feedback::modules.feedbackTags')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
    ...
    <div class="content-wrapper">
        <div class="row row-cols-lg-5 my-3">
            ...
            <div class="col mb-4">
                <a href="javascript:;" data-status="all" class="widget-filter-status">
                    <x-cards.widget :title="__('feedback::modules.total')" value="0" icon="ticket-alt" widgetId="totalFeedback & NPS" />
                </a>
            </div>

            <div class="col mb-4">
                <a href="javascript:;" data-status="closed" class="widget-filter-status">
                    <x-cards.widget :title="__('feedback::modules.totalClosed')" value="0" icon="ticket-alt" widgetId="closedFeedback & NPS" />
                </a>
            </div>

            <div class="col mb-4">
                <a href="javascript:;" data-status="open" class="widget-filter-status">
                    <x-cards.widget :title="__('feedback::modules.totalOpen')" value="0" icon="ticket-alt" widgetId="openFeedback & NPS" />
                </a>
            </div>

            <div class="col mb-4">
                <a href="javascript:;" data-status="pending" class="widget-filter-status">
                    <x-cards.widget :title="__('feedback::modules.totalPending')" value="0" icon="ticket-alt" widgetId="pendingFeedback & NPS" />
                </a>
            </div>

            <div class="col">
                <a href="javascript:;" data-status="resolved" class="widget-filter-status">
                    <x-cards.widget :title="__('feedback::modules.totalResolved')" value="0" icon="ticket-alt" widgetId="resolvedFeedback & NPS" />
                </a>
            </div>
        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center mt-3">
                <x-forms.link-primary :link="route('feedback.create')" class="mr-3 openRightModal float-left" icon="plus">
                    @lang('app.addNew') @lang('feedback::modules.feedback')
...
@push('scripts')
    ...
    <script>
        ...
        $('#ticket-table').on('change', '.change-status', function() {
            var url = "{{ route('feedback.change-status') }}";
            var token = "{{ csrf_token() }}";
            var id = $(this).data('ticket-id');
            var status = $(this).val();
        ...
        function refreshCount() {
            ...
            var url = "{{ route('feedback.refresh_count') }}";
            $.easyAjax({
                type: 'POST',
                url: url,
                data: {
                ...
                success: function(response) {
                    $('#totalFeedback & NPS').html(response.totalFeedback & NPS);
                    $('#closedFeedback & NPS').html(response.closedFeedback & NPS);
                    $('#openFeedback & NPS').html(response.openFeedback & NPS);
                    $('#pendingFeedback & NPS').html(response.pendingFeedback & NPS);
                    $('#resolvedFeedback & NPS').html(response.resolvedFeedback & NPS);
