        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center mt-3">
                @if ($addTicketPermission == 'all' || $addTicketPermission == 'added')
                    <x-forms.link-primary :link="route('tiket.create')" class="mr-3 openRightModal float-left"
                        icon="plus">
    ...
    <script>
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('tiket.destroy', ':id') }}";
        ...
        $('#ticket-table').on('change', '.change-status', function() {
            var url = "{{ route('tiket_client.change-status') }}";
            var token = "{{ csrf_token() }}";
            var id = $(this).data('ticket-id');
            var status = $(this).val();
        ...
        const applyQuickAction = () => {
            ...
            var url = "{{ route('tiket.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        function refreshCount() {
            ...
            var url = "{{ route('tiket_client.refresh_count') }}";
            $.easyAjax({
                type: 'POST',
                url: url,
                data: {
