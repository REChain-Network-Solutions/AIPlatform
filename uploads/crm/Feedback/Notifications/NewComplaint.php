...
class NewFeedback extends BaseNotification
{
    /**
    ...
    public function toMail($notifiable)
    {
        ...
        $content = __('email.newTicket.text') . '<br>' . ucfirst($this->feedback->subject) . ' # ' . $this->feedback->feedback_number . '<br>' . __('modules.tickets.requesterName') . ' - ' . mb_ucwords($this->feedback->requester->name);

        return parent::build()
            ->subject(__('email.newTicket.subject') . ' - ' . ucfirst($this->feedback->subject) . ' - ' . __('modules.tickets.ticket') . ' # ' . $this->feedback->feedback_number)
            ->markdown('mail.email', [
                'url' => $url,
                'content' => $content,
