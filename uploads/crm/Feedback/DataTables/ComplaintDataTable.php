...
{
    private $deleteFeedbackPermission;
    private $viewFeedbackPermission;
    private $editFeedbackPermission;
    ...
    public function __construct()
    {
        ...
        $this->editFeedbackPermission = user()->permission('edit_feedback');
    }
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if ($this->editFeedbackPermission == 'all' || $this->editFeedbackPermission == 'none' || $this->editFeedbackPermission == 'owned') {
                $action .= '<a href="' . route('feedback.show', [$row->feedback_number]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
            }

            if ($this->deleteFeedbackPermission == 'all') {
                $action .=
        ...
        $datatables->addColumn('status', function ($row) {
            if ($this->editFeedbackPermission == 'all' || $this->editFeedbackPermission == 'owned' && user()->id == $row->agent_id) {
        ...
        $datatables->editColumn('subject', function ($row) {
            return '<a href="' . route('feedback.show', $row->feedback_number) . '" class="text-darkest-grey" >' . ucfirst($row->subject) . '</a>';
        });
        $datatables->addColumn('name', function ($row) {
            return $row->requester ? $row->requester->name : $row->feedback_number;
        });
        ...
        CustomField::customFieldData($datatables, Feedback::CUSTOM_FIELD_MODEL);
    ...
    public function query(Feedback $model)
    {
        ...
        if ($this->viewFeedbackPermission == 'owned') {
            $model->where(function ($query) {
                $query->where('feedback.user_id', '=', user()->id)
                ->orWhere('feedback.agent_id', '=', user()->id);
            });
        }

        if ($this->viewFeedbackPermission == 'none') {
            $model->where(function ($query) {
                $query->where('feedback.user_id', '=', user()->id);
            });
        }
        ...
        if (!is_null($request->startDate) && $request->startDate != '') {
            ...
            $model->where(DB::raw('DATE(feedback.updated_at)'), '>=', $startDate);
        }

        if (!is_null($request->endDate) && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $model->where(DB::raw('DATE(feedback.updated_at)'), '<=', $endDate);
        }

        if (!is_null($request->agentId) && $request->agentId != 'all' && $request->ticketFilterStatus != 'unassigned') {
            $model->where('feedback.agent_id', '=', $request->agentId);
        }

        if (!is_null($request->priority) && $request->priority != 'all') {
            $model->where('feedback.priority', '=', $request->priority);
        }

        if (!is_null($request->channelId) && $request->channelId != 'all') {
            $model->where('feedback.channel_id', '=', $request->channelId);
        }

        if (!is_null($request->typeId) && $request->typeId != 'all') {
            $model->where('feedback.type_id', '=', $request->typeId);
        }

        if (!is_null($request->tagId) && $request->tagId != 'all') {
            $model->join('ticket_tags', 'ticket_tags.ticket_id', 'feedback.id');
        ...
        if (!is_null($request->ticketStatus) && $request->ticketStatus != 'all' && $request->ticketFilterStatus == '') {
            $request->ticketStatus == 'unassigned' ? $model->whereNull('agent_id') : $model->where('feedback.status', '=', $request->ticketStatus);
        }

        if ($request->ticketFilterStatus != '') {
            $request->ticketFilterStatus == 'open' || $request->ticketFilterStatus == 'unassigned'
                ? $model->where(function ($query) {
                    $query->where('feedback.status', '=', 'open')->orWhere('feedback.status', '=', 'pending');
                })
                : $model->where(function ($query) {
                    $query->where('feedback.status', '=', 'resolved')->orWhere('feedback.status', '=', 'closed');
                });
        ...
        if ($request->searchText != '') {
            $model->where(function ($query) {
                $query
                    ->where('feedback.subject', 'like', '%' . request('searchText') . '%')
                    ->orWhere('feedback.feedback_number', 'like', '%' . request('searchText') . '%')
                    ->orWhere('feedback.status', 'like', '%' . request('searchText') . '%')
                    ->orWhere('users.name', 'like', '%' . request('searchText') . '%')
                    ->orWhere('feedback.priority', 'like', '%' . request('searchText') . '%');
            });
        }
    ...
    {
        $data = [
            __('modules.tickets.ticket') . ' #' => ['data' => 'feedback_number', 'name' => 'feedback_number', 'title' => __('modules.tickets.ticket') . ' #', 'visible' => showId()],
        ...
        return array_merge($data, CustomFieldGroup::customFieldsDataMerge(new Feedback()));
    }
}
