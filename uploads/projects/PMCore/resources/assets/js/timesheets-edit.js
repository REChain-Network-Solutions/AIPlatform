$(function () {
    ...
    // Load initial tasks if project is selected
    if (pageData.timesheet.project_id) {
        loadProjectTasks(pageData.timesheet.project_id, pageData.timesheet.task_id);
    }
    ...
    function initializeFormHandlers() {
        ...
        $('#project_id').on('change', function () {
            ...
            if (projectId && projectId !== pageData.timesheet.project_id) {
     ...
     */
    function loadProjectTasks(projectId, selectedTaskId = null) {
                ...
                if (response.status === 'success') {
                    ...
                    if (response.data.tasks && response.data.tasks.length > 0) {
                        response.data.tasks.forEach(function (task) {
                            const option = new Option(task.text, task.id, false, task.id == selectedTaskId);
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: pageData.labels.updateSuccess,
