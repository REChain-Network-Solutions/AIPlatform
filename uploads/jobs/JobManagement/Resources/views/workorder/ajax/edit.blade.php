        ...
        <div class="bg-white rounded b-shadow-4 create-inv">
            ...
            <x-form class="c-inv-form" id="saveInvoiceForm"> @method('PUT')
                <div class="row px-lg-4 px-md-4 px-3 py-3">
                        ...
                        <div class="form-group mb-lg-0 mb-md-0 mb-4">
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="workrequest_id" id="workrequest_id"
                                    ...
                                    <option value="">--</option>
                                    @foreach ($wr as $items)
                                        <option @if ($items->id == $wo->workrequest_id) selected @endif
                                            value="{{ $items->id }}">{{ ucwords($items->wr_no) }}</option>
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                        </div>
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="complaint_id" id="complaint_id"
                                    ...
                                    @foreach ($ticket as $items)
                                        <option @if ($items->id == $wo->complaint_id) selected @endif
                    ...
                    <div class="col-md-3">
                        <div class="form-group mb-lg-0 mb-md-0 mb-4">
                            <x-forms.label fieldId="invoice_id" :fieldLabel="__('engineerings::app.menu.invoiceID')" fieldName="invoice_id"
                                fieldRequired="true">
                            </x-forms.label>
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="invoice_id" id="invoice_id"
                                    ...
                                    @foreach ($invoice as $items)
                                        <option @if ($items->id == $wo->invoice_id) selected @endif
                                            value="{{ $items->id }}">{{ ucwords($items->custom_invoice_number) }}
                                        </option>
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control select-picker" name="area_id" id="area_id" data-live-search="true">
                            ...
                            @foreach ($areas as $area)
                                <option @if ($area->id == $wo->house->area->id) selected @endif value="{{ $area->id }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control select-picker" name="house_id" id="house_id" data-row-id="0"
                            data-live-search="true">
                            <option value="">No Items Selected</option>
                            <option value="{{ $wo->house->house_name }}" selected>{{ $wo->house->house_name }}</option>
                        </select>
                    </div>
                    <div class="col-lg-3">
                    ...
                    <div class="col-lg-3">
                        <x-forms.label class="mt-3" fieldId="category" :fieldLabel="__('engineerings::app.menu.category')" fieldRequired="true">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category" id="category">
                                <option value="">--</option>
                                <option @if ('planned' == $wo->category) selected @endif value="planned">Planned</option>
                                <option @if ('unplanned' == $wo->category) selected @endif value="unplanned">Unplanned
                                </option>
                            </select>
                        </x-forms.input-group>
                    </div>
                    <div class="col-lg-3">
                        <x-forms.label class="mt-3" fieldId="priority" :fieldLabel="__('engineerings::app.menu.priority')" fieldRequired="true">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="priority" id="priority">
                                <option value="">--</option>
                                <option @if ('low' == $wo->priority) selected @endif value="low">Low</option>
                                <option @if ('medium' == $wo->priority) selected @endif value="medium">Medium</option>
                                <option @if ('high' == $wo->priority) selected @endif value="high">High</option>
                                <option @if ('emergency' == $wo->priority) selected @endif value="emergency">Emergency
                                </option>
                            </select>
                        </x-forms.input-group>
                    </div>
                    <div class="col-lg-3">
                        <x-forms.label class="mt-3" fieldId="status" :fieldLabel="__('engineerings::app.menu.status')" fieldRequired="true">
                        </x-forms.label>
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="status" id="status">
                                <option value="">--</option>
                                <option @if ('incomplete' == $wo->status) selected @endif value="incompleted">Incompleted
                                </option>
                                <option @if ('pending' == $wo->status) selected @endif value="pending">Pending
                                </option>
                                <option @if ('completed' == $wo->status) selected @endif value="completed">Completed
                                </option>
                            </select>
                        </x-forms.input-group>
                    </div>
                        ...
                        <div class="form-group mb-lg-0 mb-md-0 mb-4">
                            ...
                            <x-forms.input-group class="border px-2">
                                ...
                                <input type="text" name="estimate_hours" id="estimate_hours"
                                    ...
                                    value="{{ $wo->estimate_hours }}" readonly>
                                ...
                                <input type="text" name="estimate_minutes" id="estimate_minutes"
                                    ...
                                    value="{{ $wo->estimate_minutes }}" readonly>
                            </x-forms.input-group>
                        </div>
                    </div>
                        ...
                        <div class="form-group mb-lg-0 mb-md-0 mb-4">
                            ...
                            <x-forms.input-group class="border px-2">
                                ...
                                <input type="text" name="actual_hours" id="actual_hours"
                                    class="form-control-plaintext height-35 f-15 px-2 border-right"
                                    value="{{ $wo->actual_hours }}" readonly>
                                <label class="px-2">@lang('engineerings::app.menu.min'): </label>
                                <input type="text" name="actual_minutes" id="actual_minutes"
                                    class="form-control-plaintext height-35 f-15 px-2" value="{{ $wo->actual_minutes }}"
                                    readonly>
                            </x-forms.input-group>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="work_description" id="description-text" rows="5" class="form-control">{{ $wo->work_description }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <x-forms.label class="my-3" fieldId="description-text" :fieldLabel="__('engineerings::app.menu.completitionNotes')"
                                fieldRequired="true">
                            </x-forms.label>
                            <textarea name="completion_notes" id="description-text" rows="5" class="form-control">{{ $wo->completion_notes }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <x-form-actions class="c-inv-btns d-block d-lg-flex d-md-flex">
                    <x-forms.button-primary data-type="save" class="save-form mr-3" icon="check">@lang('app.save') Work
                        Order
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.save-form').click(function() {
                const url = "{{ route('work.update', $wo->id) }}";
                $.easyAjax({
                    url: url,
