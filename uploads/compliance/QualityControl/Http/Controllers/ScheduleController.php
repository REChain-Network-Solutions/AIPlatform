...
class ScheduleController extends AccountBaseController
{
    ...
    {
        $viewPermission = user()->permission('view_quality_control');
    ...
    public function create()
    {
        ...
        $this->addPermission = user()->permission('add_quality_control');
    ...
    public function edit($id)
    {
        ...
        $this->editPermission = user()->permission('edit_quality_control');
    ...
    public function show($id)
    {
        ...
        $this->viewPermission = user()->permission('view_quality_control');
        $this->addSchedulesPermission = user()->permission('add_quality_control');
    ...
    public function destroy($id)
    {
        ...
        $schedule = Schedule::findOrFail($id);
        $this->deletePermission = user()->permission('delete_quality_control');
        abort_403(!(
    ...
    /**
     * Ensure there is a linked Quality Issue (Complaint module) for this inspection schedule.
     * This is a light integration: it simply dispatches the same event used by the checklist
     * completion logic (needs_reclean). The Complaint module will create the issue if not exists.
     */
    public function createQualityIssue($scheduleId)
    {
        ...
        if (!empty($schedule->complaint_id) && \Illuminate\Support\Facades\Route::has('complaint.show')) {
            return redirect()->route('complaint.show', $schedule->complaint_id);
        }

        \Illuminate\Support\Facades\Event::dispatch('quality_control.needs_reclean', [
            'company_id' => $schedule->company_id,
            'user_id' => $schedule->user_id ?? user()->id,
            'job_id' => $schedule->job_id ?? null,
            'quality_control_id' => $schedule->id,
            'reason' => 'manual_create_issue',
        ]);

        // Reload to see if listener backfilled complaint_id.
        $schedule->refresh();

        if (!empty($schedule->complaint_id) && \Illuminate\Support\Facades\Route::has('complaint.show')) {
            return redirect()->route('complaint.show', $schedule->complaint_id)
                ->with('success', 'Quality Issue created.');
        }

        return redirect()->back()->with('success', 'Requested Quality Issue creation.');
    }


    /**
     * Set QC outcome for an inspection schedule.
     * Allowed outcomes: pending, pass, fail, needs_reclean, escalated
     */
    public function setOutcome(Request $request, Schedule $schedule)
    {
        $editPermission = user()->permission('edit_quality_control');
        ...
        if (in_array($schedule->qc_outcome, ['fail', 'needs_reclean', 'escalated'])) {
            try {
                if (method_exists($this, 'createQualityIssue')) {
                    // createQualityIssue expects the schedule id via route-model binding - call internal helper
                    // We don't redirect here; we just ensure complaint_id is present.
                    $this->ensureQualityIssue($schedule);
                }
            } catch (\Throwable $e) {
                // Silent fail - outcome saving should never crash.
            }
        }
    ...
    /**
     * Ensure a linked Quality Issue exists for a schedule (idempotent).
     */
    protected function ensureQualityIssue(Schedule $schedule): void
    {
        if (!empty($schedule->complaint_id)) {
            return;
        }

        // Complaint module is optional. If missing, do nothing.
        ...
        // Minimal required fields: subject, description, status, priority, user/company scoping handled by BaseModel/HasCompany.
        $complaint->subject = $schedule->subject ?: 'Quality Issue';
        $complaint->status = 'open';
        $complaint->priority = 'high';
        $complaint->user_id = user()->id;
        $complaint->agent_id = $schedule->agent_id ?? null;
        $complaint->quality_control_id = $schedule->id;
        $complaint->quality_control_reason = 'Auto-created from inspection outcome: ' . $schedule->qc_outcome;
        $complaint->job_id = $schedule->job_id ?? null;

        if (property_exists($complaintClass, 'CUSTOM_FIELD_MODEL')) {
            // no-op, but keeps compatibility with some installs
        }

        $complaint->save();

        $schedule->complaint_id = $complaint->id;
        $schedule->save();
    }
