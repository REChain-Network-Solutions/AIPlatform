                        ...
                        <table class="inv-num-date text-dark f-13 mt-3">
                            ...
                            <tr>
                                ...
                                <td class="border-left-0">
                                    <form id="qcOutcomeForm" class="d-flex align-items-center" method="POST" action="{{ route('inspection_schedules.set_outcome', $schedule->id) }}">
                                        @csrf
                                        <select name="qc_outcome" class="form-control form-control-sm mr-2" style="max-width: 220px;">
                                            @php($currentOutcome = $schedule->qc_outcome ?? 'pending')
                                            <option value="pending" @selected($currentOutcome=='pending')>Pending</option>
                                            <option value="pass" @selected($currentOutcome=='pass')>Pass</option>
                                            <option value="fail" @selected($currentOutcome=='fail')>Fail</option>
                                            <option value="needs_reclean" @selected($currentOutcome=='needs_reclean')>Needs Re-clean</option>
                                            <option value="escalated" @selected($currentOutcome=='escalated')>Escalated</option>
                                        </select>
                            ...
                            @if (\Illuminate\Support\Facades\Route::has('complaint.show'))
                                <tr>
                                    <td class="bg-light-grey border-right-0 f-w-500">
                                        Quality Issue</td>
                                    <td class="border-left-0">
                                        @if (!empty($schedule->complaint_id))
                                            <a href="{{ route('complaint.show', $schedule->complaint_id) }}" class="btn btn-sm btn-outline-dark">
                                                View Issue #{{ $schedule->complaint_id }}
                                            </a>
                                        @else
                                            <a href="{{ route('inspection_schedules.create_quality_issue', $schedule->id) }}" class="btn btn-sm btn-outline-primary">
                                                Create Issue
                                            </a>
                                        @endif
                                    </td>
                            ...
                            <tr>
                                <td class="bg-light-grey border-right-0 f-w-500">
                                    Follow-up Inspection</td>
                                <td class="border-left-0">
                                    @if (!empty($schedule->follow_up_schedule_id))
                                        <a href="{{ url('account/inspection_schedules/'.$schedule->follow_up_schedule_id) }}" class="btn btn-sm btn-outline-dark">
                                            View Follow-up #{{ $schedule->follow_up_schedule_id }}
                                        </a>
                                    @else
...
document.addEventListener('DOMContentLoaded', function () {
    var form = document.getElementById('qcOutcomeForm');
    if (!form) return;
    form.addEventListener('submit', function (e) {
        // allow normal submit if $.ajax not present
        if (typeof window.$ === 'undefined' || typeof $.ajax === 'undefined') return;
        e.preventDefault();
        $.ajax({
            url: form.action,
            type: 'POST',
