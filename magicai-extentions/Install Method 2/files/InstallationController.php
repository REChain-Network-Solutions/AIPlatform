...
/**
 * Trigger the installation of all downloaded but not fully installed extensions.
 *
 * @return \Illuminate\Http\RedirectResponse
 */
     public function installAllExtensions()
    {
        $extensions = \App\Models\Extension::where('installed', 0)->get();
        $results = [];

        foreach ($extensions as $extension) {
            $slug = $extension->slug; // Capture the slug here
            try {
                $this->extensionService->install($slug);
                $results[$slug] = ['status' => 'success', 'message' => "Extension {$slug} installed successfully."];
            } catch (Exception $e) {
                Log::error("Failed to install extension {$slug}: " . $e->getMessage());
                $results[$slug] = ['status' => 'error', 'message' => "Failed to install extension {$slug}: " . $e->getMessage()];
            }
        }

        return View::make('installation.results', ['results' => $results]);
    }
